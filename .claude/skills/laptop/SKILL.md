...
Send a one-line message to the remote host `laptop`'s active graphical session
via its `xmsg` popup tool:

```
MESSAGE="..."
ssh laptop "DISPLAY=:0 python3 /home/fmannella/bin/xmsg -t \"$MESSAGE\"" > /tmp/xmsg_last.log 2>&1 &
disown
```
...
5. Compose one concise line — mention the root/params if identifiable from
   ...
   for `match_std`/`decaying_speed`/`local_decaying_speed`), and the
   progress. **Always include a predicted end date/time** (absolute, e.g.
   "ETA ~15:38 CEST" or "~2026-07-07 08:40 CEST" if it crosses midnight),
   computed from current time + the remaining-time estimate above — not just
   a relative "X% done". If no run is active, skip the ETA. Send the result
   as `$MESSAGE`.
