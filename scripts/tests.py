...
def radians_to_degrees(radians):
    # Mirrors src/test.py's own conversion: object_params["rot"] is saved
    # into the goal filename in degrees, not the raw --posrot CLI value.
...
def goals_filename(shape, rotation):
    ...
    parts = [f"{POS}_", f"{radians_to_degrees(rotation):06.2f}_"]
